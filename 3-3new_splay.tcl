...
append outfile2  $number "splay3-3.dat"
...
for {set nn 0} {$nn < 200} {incr nn 1} {
	puts "frame:   $nn"
	set p_to_calculate [atomselect top "resname $lip3 and name O3" frame $nn]

	for {set i 0} {$i < [$p_to_calculate num]} {incr i} {
		set p_index [lindex [$p_to_calculate get resid] $i]
		set ctail_sel [atomselect top "resid $p_index and name C3" frame $nn]
		set ctail [measure center $ctail_sel]
		set pnhead_sel [atomselect top "resid $p_index and name C17" frame $nn]
		set pnhead [measure center $pnhead_sel ]
		set t [vecnorm [vecsub $ctail $pnhead]]
		set p_to_calculate2 [atomselect top "(resname $lip3 and name C3) and (within 10.0 of (resid $p_index and name C3)) and not (resid $p_index)" frame $nn]
		puts "num surrounding [$p_to_calculate2 num]"
		set res_list2 [$p_to_calculate2 get resid]
		foreach id_res2 $res_list2 {
			set pnhead2_sel [atomselect top "resid $id_res2 and name C17" frame $nn]
			set pnhead2 [measure center $pnhead2_sel]
			set ctail2_sel [atomselect top "resid $id_res2 and name C3" frame $nn]			
